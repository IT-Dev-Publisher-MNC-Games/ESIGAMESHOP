...
$(document).ready(function () {
  ...
  const checkError = ({ prefixChild, extendMessage = '' }) => {
    const errors = document.getElementsByClassName('errors')[0];
    if (!errors) return;
    const messageError = errors.querySelector('.' + errors.className + prefixChild).innerText;
    if (errors.hasChildNodes(this.className + prefixChild)) {
  ...
  checkError({ prefixChild: '__captcha', extendMessage: 'Please try again' });
      ...
      await fetch(urlPayment, {
        ...
        .then((data) => {
          ...
          $("#nav-tab .nav-link").click(function () {
            ...
            $(".price-list").empty();
            $(".payment-list__items .container").children().prop("checked", false);
            clearItems();
          });
...
const addTabContent = ({ category_id, payment_id, name_channel, logo_channel }) => {
  $("#nav-" + category_id + " .payment-list").append(`<div class="col"><div class="payment-list__items" data-payment="${payment_id}"><label class="container"><input type="radio" id="${payment_id}" name="payment-id" value="${payment_id}"><span class="checkmark"></span><img src="${logo_channel}" title="${name_channel}" alt="${name_channel}" class="ps-2"></label></div></div>`);
  return;
}
